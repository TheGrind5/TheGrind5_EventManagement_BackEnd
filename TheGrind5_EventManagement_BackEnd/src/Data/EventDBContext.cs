...
public partial class EventDBContext : DbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder b)
    {
        ...
        b.Entity<Product>().ToTable("Product");
        b.Entity<OrderProduct>().ToTable("OrderProduct");
        b.Entity<Feedback>().ToTable("Feedback");
        b.Entity<FeedbackReaction>().ToTable("FeedbackReaction");
        ...
        ConfigureProductRelationships(b);
        ConfigureFeedbackRelationships(b);
    ...
    private void ConfigureProductRelationships(ModelBuilder b)
    {
        // Product -> Event : required, cascade delete
        b.Entity<Product>()
         .HasOne(p => p.Event)
        ...
        // Cấu hình Primary Key cho Product
        b.Entity<Product>()
    ...
    private void ConfigureFeedbackRelationships(ModelBuilder b)
    {
        // Feedback -> Event : required, không cascade
        b.Entity<Feedback>()
         .HasOne(f => f.Event)
        ...
        // Feedback -> User : required, không cascade
        b.Entity<Feedback>()
         .HasOne(f => f.User)
        ...
        // Feedback -> ParentFeedback : optional, không cascade
        b.Entity<Feedback>()
        ...
        // FeedbackReaction -> Feedback : required, cascade delete
        b.Entity<FeedbackReaction>()
         .HasOne(fr => fr.Feedback)
         .WithMany(f => f.Reactions)
         .HasForeignKey(fr => fr.FeedbackId)
         .OnDelete(DeleteBehavior.Cascade);

        // FeedbackReaction -> User : required, không cascade
        b.Entity<FeedbackReaction>()
         .HasOne(fr => fr.User)
         .WithMany(u => u.FeedbackReactions)
         .HasForeignKey(fr => fr.UserId)
         .OnDelete(DeleteBehavior.Restrict);

        // Cấu hình Primary Key cho FeedbackReaction
        b.Entity<FeedbackReaction>()
         .HasKey(fr => fr.ReactionId);

        // Unique constraint: mỗi user chỉ có 1 reaction cho mỗi feedback
        b.Entity<FeedbackReaction>()
         .HasIndex(fr => new { fr.FeedbackId, fr.UserId })
         .IsUnique();
    }
}
