-- ============================================
-- THE GRIND 5 EVENT MANAGEMENT - OPTIMIZED DATABASE
-- ============================================
-- Optimized version with normalized table structure
-- Separated large tables into smaller, focused tables
...
IF DB_ID('TheGrind5EventDB_Optimized') IS NOT NULL
BEGIN
    ALTER DATABASE TheGrind5EventDB_Optimized SET SINGLE_USER WITH ROLLBACK IMMEDIATE;
    DROP DATABASE TheGrind5EventDB_Optimized;
END;
GO
CREATE DATABASE TheGrind5EventDB_Optimized COLLATE Vietnamese_CI_AI;
GO
USE TheGrind5EventDB_Optimized;
GO

-- ============================================
-- CORE USER MANAGEMENT (OPTIMIZED)
-- ============================================

-- Core User table (essential fields only)
CREATE TABLE [User](
...
CREATE TABLE UserPreferences(
    ...
    PrivacySettings NVARCHAR(MAX), -- JSON for privacy settings
    ThemeSettings NVARCHAR(MAX), -- JSON for UI theme preferences
    LanguageSettings NVARCHAR(MAX), -- JSON for language preferences
    SocialMediaLinks NVARCHAR(MAX), -- JSON for social media links
    CreatedAt DATETIME2(0) NOT NULL DEFAULT SYSDATETIME(),
    UpdatedAt DATETIME2(0),
    CONSTRAINT FK_UserPreferences_User FOREIGN KEY (UserId) REFERENCES [User](UserId) ON DELETE CASCADE,
    CONSTRAINT UQ_UserPreferences_User UNIQUE (UserId)
);
...
-- ============================================
-- ENHANCED EVENT MANAGEMENT (OPTIMIZED)
-- ============================================
...
-- Core Event table (essential fields only)
CREATE TABLE Event(
...
-- Event Location (location-specific information)
CREATE TABLE EventLocation(
    ...
    CONSTRAINT FK_EventLocation_Event FOREIGN KEY (EventId) REFERENCES Event(EventId) ON DELETE CASCADE,
    CONSTRAINT UQ_EventLocation_Event UNIQUE (EventId)
);

-- Event Content (content and media)
CREATE TABLE EventContent(
    ...
    CONSTRAINT FK_EventContent_Event FOREIGN KEY (EventId) REFERENCES Event(EventId) ON DELETE CASCADE,
    CONSTRAINT UQ_EventContent_Event UNIQUE (EventId)
);
...
-- ============================================
-- ENHANCED TICKET MANAGEMENT (OPTIMIZED)
-- ============================================

-- Core TicketType (essential fields)
...
-- TicketType Settings (additional settings)
CREATE TABLE TicketTypeSettings(
    ...
    RefundPolicy NVARCHAR(MAX),
    SpecialInstructions NVARCHAR(MAX),
    CONSTRAINT FK_TicketTypeSettings_TicketType FOREIGN KEY (TicketTypeId) REFERENCES TicketType(TicketTypeId) ON DELETE CASCADE,
    CONSTRAINT UQ_TicketTypeSettings_TicketType UNIQUE (TicketTypeId)
);
...
PRINT 'Optimized The Grind 5 Event Management Database created successfully!';
PRINT 'Total tables created: 35+ (optimized structure)';
PRINT 'All workflows (Admin, Host, Customer) are fully supported';
PRINT 'Database is optimized for better performance and maintainability';
