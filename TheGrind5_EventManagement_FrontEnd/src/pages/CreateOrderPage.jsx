...
const CreateOrderPage = () => {
    ...
    const pricing = React.useMemo(() => {
        ...
        // Tính tổng giá vé
        const ticketAmount = ticketType.price * quantity;
        
        // Tính tổng giá phụ kiện
        const accessoriesAmount = selectedProducts.reduce((total, product) => total + (product.price * product.quantity), 0);
        
        // Tổng cộng = vé + phụ kiện
        const originalAmount = ticketAmount + accessoriesAmount;
        let finalAmount = originalAmount;
        let discountAmount = 0;

        if (appliedVoucher) {
            // Tính toán lại discount dựa trên tổng mới (vé + phụ kiện)
            if (appliedVoucher.discountPercentage) {
                discountAmount = Math.round(originalAmount * appliedVoucher.discountPercentage / 100);
                finalAmount = originalAmount - discountAmount;
            } else if (appliedVoucher.discountAmount) {
                discountAmount = appliedVoucher.discountAmount;
                finalAmount = originalAmount - discountAmount;
            } else {
                // Fallback to original logic if no percentage/amount
                discountAmount = appliedVoucher.discountAmount || 0;
                finalAmount = appliedVoucher.finalAmount || originalAmount;
            }
        }
    ...
    }, [selectedTicketType, ticketTypes, quantity, appliedVoucher, selectedProducts]);
                                ...
                                {selectedTicketType && quantity > 0 && pricing && (
                                    <div className="form-group">
                                        <div className="alert alert-info">
                                            ...
                                            <p><strong>Tổng tiền vé:</strong> {pricing.ticketAmount.toLocaleString()} VND</p>
                                            
                                            {/* Hiển thị phụ kiện đã chọn */}
                                            {selectedProducts.length > 0 && (
                                                <div className="mt-3">
                                                    <h6>🛍️ Phụ kiện đã chọn:</h6>
                                                    {selectedProducts.map((product, index) => (
                                                        <p key={index} className="mb-1">
                                                            <strong>{product.productName}</strong> x{product.quantity} - {product.price.toLocaleString()} VND
                                                        </p>
                                                    ))}
                                                    <p><strong>Tổng phụ kiện:</strong> {pricing.accessoriesAmount.toLocaleString()} VND</p>
                                                </div>
                                            )}
