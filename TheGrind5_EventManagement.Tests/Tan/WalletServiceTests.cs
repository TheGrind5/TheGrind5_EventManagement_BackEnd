...
namespace TheGrind5_EventManagement.Tests.UnitTests
{
    public class WalletServiceTests : IDisposable
    {
        ...
        [Fact]
        public async Task GetWalletBalanceAsync_WithValidUser_ShouldReturnBalance()
        {
            // Arrange
            var userId = 1;
            var user = new User
        ...
        [Fact]
        public async Task GetWalletBalanceAsync_WithInvalidUser_ShouldReturnZero()
        {
            // Arrange
            var invalidUserId = 999;

            // Act
            var result = await _walletService.GetWalletBalanceAsync(invalidUserId);
